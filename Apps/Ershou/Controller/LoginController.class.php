    ...
    //更新user_id
      public function updateUserId(){
        ...
        if (!empty($phpsessid)) {
            ...
            $map['user_id']=0;
            $counts=M('cart')->where($map)->count();
            if ($counts != 0) {
              $data['user_id']=$user_id;
              M('cart')->where($map)->save($data);
        ...
        if(IS_AJAX) {
            ...
            if ($verifyCode == $codeInfo['code']) {
                ...
                if (strtotime('+2 minute', $codeInfo['time']) > $curTime) {
                    ...
                    //写入登录信息
                    $usersModel = D("Common/Users");
                    $usersModel->setLoginCookie($userInfo['uid'],'mobile', $userInfo['group_id'], $data['reg_time'], $data['reg_ip'], $userInfo['mobile'], $userInfo['user_name']);
                    //再次获取用户信息
                    if ($flag) {
                        //更新购物车user_id
                        $this->updateUserId();
