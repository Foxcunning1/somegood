...
class BaseController extends Controller {
        ...
        public function checkStoreSessionAction() {
            if(!session('store.id')||!session('store.mobile')||!session('store.gid')){
                if(!$this->checkCookiesAction()){
                    session(null);
                    cookie('user_id',null);
                    cookie('user_pw',null);
                    cookie('mobile',null);
                    $this->error('当前用户未登录或登录超时，请重新登录', U('Store/Login/index'));
                }
            }
        }
        ...
        if(cookie('user_id')!=null&&cookie('?user_pw')!=null&&cookie('?mobile')!=null&&cookie('?gid')!=null){
            ...
            if($usersModel->where("uid=$id")->count()>0){
                ...
                if(md5(md5($password).$user['salt'])==$user['password']&&$mobile==$user['mobile']){
                    ...
                    session('mobile.mobile',$user['mobile']);
                    session('mobile.gid',$user['gid']);
                    return true;
                }else{
                    return false;
                }
