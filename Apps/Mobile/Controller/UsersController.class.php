...
class UsersController extends MemberController {
    ...
    public function comfirmAction(){
        ...
        if ($rs) {
            ...
            $mobile=$orderModel->alias('o')->join("LEFT JOIN __USERS__ AS u ON o.seller_id = u.uid")->where("id=$id")->getField('u.mobile');
            $rs=send_sms_message('sell', $mobile,'',array($day));
            //给店铺发送短信
            $storeMobile=$orderModel->alias('o')->field('s.mobile')
            ->join("LEFT JOIN __ORDER_GOODS__ AS mog ON mog.order_sn = o.order_sn")
            ->join("LEFT JOIN __STORE__ AS s ON s.sid = mog.store_id")->where("o.id=$id")->select();
            foreach ($storeMobile as $key => $v) {
    ...
    public function applyAction()
    {
        ...
        if ($is_seller) {
            ...
            $action = 'add';
                            ...
                            /**获取企业类型*/
                           /* if (!F('Mobile/IndustrysType/type_list')) {
                                $cacheIndustrysTypeList = require_once("./Cache/industrys/industrys_type.cache.php");
                                F('Mobile/IndustrysType/type_list', $cacheIndustrysTypeList);
                            }
                            $this->assign('industrysTypeList', F('Mobile/IndustrysType/type_list'));*/
        if(IS_POST) {
            $data = I('post.data');
            //验证post数据是否合法
            /**PC端申请,数据统一*/
        ...
        if (IS_POST){
            ...
            if($data['mobile']){
                if(!preg_match("/^1[34578]\d{9}$/", $data['mobile'])){
                    ...
                    //验证手机号是否已被绑定
                    $re=D('Common/Users')->isExits($data['mobile'],'mobile');
                    if($re){
            ...
            if($data['user_name']){
                //验证user_name是否已存在
                $re=D('Common/Users')->isExits($data['user_name'],'username');
                if($re){
                    $this->ajaxInfoReturn('','用户名已存在,请重试',0);
                }
            }
            //验证邮箱
            if($data['email']){
