...
function initWebUploader(webpath,uploadUrl){
    ...
    uploader.on('uploadSuccess',function(file,data){
    	if(data.data==0){
			...
			jcrop_api.setImage(data.data, function () {
				var fileExtension= data.data.split(".");
				var extension ='';
				if(fileExtension.length>0){
					extension= fileExtension[1];
				}
				$('#hideFileName').val(data.data);
				$('#target').attr('src', data.data);
				$('#preview').attr('src', data.data);
				$('#hideFileExtension').val(extension);
